...
Предполагаемая роль — гипотеза для проверки человеком, а не заключение о нарушении.

> **Тестовая версия AI:** ветка `codex/ai-assistant-test`, основа — проверенный `main` (`3940d94`) плюс AI-модуль `ec48a84` и подключение API/UI. В карточке есть вкладка «AI-помощник», кнопка «Объяснить с AI» и ссылки на факты. Команды ниже загружают тестовую ветку. Версия для сдачи остаётся в `main`, пока качество живых записок не принято. По умолчанию внешние вызовы выключены. [Запуск и отключение AI](docs/ai-assistant.md#запуск-и-быстрое-отключение).
...
```bash
git clone --branch codex/ai-assistant-test https://github.com/BAITC-Hacks/hack-6d0b8f9a-dofp.git moneygraph
cd moneygraph
```
...
**Без Git:** откройте ветку `codex/ai-assistant-test` в GitHub → **Code → Download ZIP**, распакуйте архив и откройте терминал в папке, где находятся `README.md`, `pyproject.toml`, `moneygraph/` и `frontend/`.
...
**AI-помощник:** в [тестовой ветке `codex/ai-assistant-test`](https://github.com/BAITC-Hacks/hack-6d0b8f9a-dofp/tree/codex/ai-assistant-test) подключены API и вкладка в карточке клиента. Записка содержит ссылки на исходные факты и переходы к операциям и цепочкам. Основной `main` остаётся без AI. Внешняя передача в OpenAI разрешена пользователем; ключ вводится локально. Живой запрос выполнен, однако первая записка отклонена проверкой утверждений: качество ответов ещё не принято. Проверка фактов не ослаблялась. [Запуск и быстрое отключение](docs/ai-assistant.md#запуск-и-быстрое-отключение).

## Что проверено

В этой тестовой AI-ветке совместно прошли **259 тестов и 36 подслучаев**: основной
