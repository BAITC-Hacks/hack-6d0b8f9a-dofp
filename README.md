...
**AI-помощник:** в `main` подключены API и вкладка в карточке клиента. Записка содержит ссылки на исходные факты и переходы к операциям и цепочкам. AI остаётся экспериментальным и отключается настройками запуска. Внешняя передача в OpenAI разрешена пользователем; ключ вводится локально. Живой запрос выполнен, однако живые записки отклонены проверкой утверждений или формата: качество ответов ещё не принято. Проверка фактов не ослаблялась. [Запуск и быстрое отключение](docs/ai-assistant.md#запуск-и-быстрое-отключение).
