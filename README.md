...
> **Тестовая версия AI:** ветка `codex/ai-assistant-test`, основа — проверенный `main` (`3940d94`) плюс AI-модуль `ec48a84`. Команды ниже загружают тестовую ветку. Версия для запуска и сдачи остаётся в `main`. Endpoint и кнопка AI ещё не подключены; здесь доступны модуль, совместные тесты и [инструкция его приёмки](docs/ai-assistant.md#интеграционная-проверка-в-отдельной-ветке). По умолчанию AI выключен.
...
**Отдельное развитие:** [AI-помощник](https://github.com/BAITC-Hacks/hack-6d0b8f9a-dofp/tree/codex/ai-assistant) реализован как backend-модуль объяснения фактов со ссылками на их источники. Для проверки вместе с приложением выделена [тестовая ветка `codex/ai-assistant-test`](https://github.com/BAITC-Hacks/hack-6d0b8f9a-dofp/tree/codex/ai-assistant-test). AI-модуль не включён в `main`; endpoint и кнопка пока не подключены. В конфигурации предусмотрен профиль OpenAI, но качество реальных ответов модели ещё не принято. Для обязательного сценария ТЗ он не требуется: AI-ассистент указан в опциональных функциях. Перед включением нужны подтверждение допустимости внешней передачи и доступа командного API-проекта, интеграция интерфейса и приёмка на настоящей модели.

## Что проверено

В этой тестовой AI-ветке совместно прошли **252 теста и 36 подслучаев**: основной
набор ниже плюс 85 проверок AI. Внешнюю модель не вызывали; endpoint и кнопка
AI пока не подключены. Подробнее — [приёмка AI](docs/ai-assistant.md#интеграционная-проверка-в-отдельной-ветке).
